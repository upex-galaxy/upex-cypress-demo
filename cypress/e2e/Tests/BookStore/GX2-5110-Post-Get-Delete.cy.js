...
import { login } from '@pages/BookStore/GX2-5110-Login';
import { profile } from '@pages/BookStore/GX2-5110-Profile';
describe('BookStore | Account | Crear, Obtener y Eliminar Usuario (POST-GET-DELETE)', () => {
	...
	it('5111 | TC2: Validate that the user tries to register a new user by inserting invalid or null data in any of the fields', () => {
	...
	it('5111 | TC4: Validate that the user tries to log in with invalid credentials', () => {
		const invalidPass = 'dfkdiir1121';
		cy.visit('/login');
		login.get.title().then(text => {
			expect(text.text()).to.equal('Welcome,Login in Book Store');
		});
		login.logIn(user, invalidPass);
		login.clickLoginButton();
		login.get.outputWrapper().within(() => {
			login.get.output().should('have.text', 'Invalid username or password!');
		});
	});
	it('5111 | TC5: Validate that the user logs out', () => {
		cy.visit('/login');
		login.logIn(user, pass);
		login.clickLoginButton();
		profile.get.booksWrapper().within(() => {
			profile.get.userNameLabel().should('have.text', 'User Name : ');
	...
	it('5111 | TC6: Validate that the user can check the contents of the collections', () => {
		cy.request({
			method: 'POST',
			url: '/Account/v1/GenerateToken',
			body: {
	...
	it('5111 | TC7: Validate that the user deletes his/her account', () => {
		cy.request({
			method: 'POST',
			url: '/Account/v1/GenerateToken',
			body: {
