describe('TS#27520 : ✅ToolsQA | Elements | Text Box: Fill form and Submit', () => {
	...
	it('US # | TC#4: 27520 | Validate not submit form when email not contain (minimum) 1 alphanumeric character before “@”', () => {
		cy.fixture('data/GX-27519-TextBox').then(the => {
			//complete Form
			cy.get(the.fullName.input).type(the.fullName.data.stringSpace);
			cy.get(the.fullName.input).should('have.value', the.fullName.data.stringSpace);
			cy.get(the.email.input).type(the.email.data.invalid.not_At_Char);
			cy.get(the.email.input).should('have.value', the.email.data.invalid.not_At_Char);
			cy.get(the.currentAddress.input).type(the.currentAddress.data.stringSpace);
			cy.get(the.currentAddress.input).should('have.value', the.currentAddress.data.stringSpace);
			cy.get(the.permanentAddress.input).type(the.permanentAddress.data.stringSpace);
			cy.get(the.permanentAddress.input).should('have.value', the.permanentAddress.data.stringSpace);
			//click Submit
			cy.get(the.buttonSubmit).click();
			//assert
			cy.get(the.email.input).should('have.css', 'border', '1px solid rgb(255, 0, 0)');
	...
	it('US # | TC#5: 27520 | Validate not submit form when email not contain (minimum) 1 alphanumeric character after “@”', () => {
		cy.fixture('data/GX-27519-TextBox').then(the => {
			//complete Form
			cy.get(the.fullName.input).should('be.empty');
			cy.get(the.email.input).type(the.email.data.invalid.not_Char_At);
			cy.get(the.email.input).should('have.value', the.email.data.invalid.not_Char_At);
			cy.get(the.currentAddress.input).should('be.empty');
			cy.get(the.permanentAddress.input).should('be.empty');
			//click Submit
			cy.get(the.buttonSubmit).click();
	...
	it('US # | TC#6: 27520 | Validate not submit form when email not contain “.” after: 1 alphanumeric character after “@”', () => {
		cy.fixture('data/GX-27519-TextBox').then(the => {
			//complete Form
			cy.get(the.fullName.input).should('be.empty');
			cy.get(the.email.input).type(the.email.data.invalid.not_At_Char_Point);
			cy.get(the.email.input).should('have.value', the.email.data.invalid.not_At_Char_Point);
			cy.get(the.currentAddress.input).should('be.empty');
			cy.get(the.permanentAddress.input).should('be.empty');
			//click Submit
			cy.get(the.buttonSubmit).click();
	...
	it('US # | TC#7: 27520 | Validate not submit form when email not contain (minimum) 2 alphanumeric characters after “.”', () => {
		cy.fixture('data/GX-27519-TextBox').then(the => {
			//complete Form
			cy.get(the.fullName.input).should('be.empty');
			cy.get(the.email.input).type(the.email.data.invalid.not_Point_Char_Char);
			cy.get(the.email.input).should('have.value', the.email.data.invalid.not_Point_Char_Char);
			cy.get(the.currentAddress.input).should('be.empty');
			cy.get(the.permanentAddress.input).should('be.empty');
			//click Submit
			cy.get(the.buttonSubmit).click();
