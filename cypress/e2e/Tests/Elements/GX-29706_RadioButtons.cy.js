...
describe('GX-29706: ToolsQA | Elements | Radio Buttons', () => {
	...
	it('TC2: Validar visualizar el label Impressive cuando se selecciona el radio button Impresive', () => {
		cy.get('#impressiveRadio').click({ force: true });
		cy.contains('You have selected ').children().should('have.text', 'Impressive');
	});
	it('TC3: Validar que el radio button con label "NO" esté deshabilitado y no se pueda seleccionar', () => {
