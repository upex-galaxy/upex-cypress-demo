...
describe('ToolsQA | Elements | Radio Buttons', () => {
	...
	it('GX-30382|TC1:Validar visualizar el label YES cuando se seleciona el radio button YES  ', () => {
		cy.get('#yesRadio').click({ force: true });
		cy.contains('You have selected').children().should('have.text', 'Yes');
	});
	it('GX-30382|TC2:Validar visualizar el label impressive cuando se seleciona el radio button impresivve ', () => {
		cy.get('#impressiveRadio').click({ force: true });
		cy.contains('You have selected').children().should('have.text', 'Impressive');
	});

	it('GX-30382|TC3:Validar que el radio button con label no este deshabilitado y no se puede seleccionar ', () => {
