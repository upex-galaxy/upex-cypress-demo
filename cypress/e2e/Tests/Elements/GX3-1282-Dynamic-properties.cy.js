...
describe('US GX3-1282| ToolsQA | Elements | Dynamic Properties', () => {
	...
	it('GX3-1289|TC001 Validate being able to get item with Dymanic ID', () => {
		cy.get('p').should('contain', 'This text has random Id');
	});

	it('GX3-1289|TC002 Validate that the item changes state to enabled after 5 seconds', () => {
		cy.get('#enableAfter').should('be.disabled');
		cy.get('#enableAfter', { timeout: 5000 }).should('be.enabled');
	});

	it('GX3-1289|TC003 Validate that the color changes after 5 seconds', () => {
		cy.get('#colorChange', { timeout: 5000 }).should('have.css', 'color', 'rgb(220, 53, 69)');
	});

	it('GX3-1289|TC004 Validate that the button is visible after 5 seconds', () => {
