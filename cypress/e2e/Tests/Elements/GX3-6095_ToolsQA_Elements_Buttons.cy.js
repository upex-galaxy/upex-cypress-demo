describe('GX3-6095 | ToolsQA | Elements | Buttons', () => {
	...
	it('GX3-6095 | TC1: Validate “Click Me” Button is working successfully. (HP)', () => {
		cy.get("[type = 'button'].btn.btn-primary").eq(2).click();
	...
	it('GX3-6095 | TC4: Validate that no results are returned with different actions on the “Double Click Me” Button', () => {
		cy.get("[type = 'button'].btn.btn-primary").eq(2).should('be.visible').dblclick();
		cy.get("[type = 'button'].btn.btn-primary").eq(2).should('be.visible').rightclick();
		cy.get('#dynamicClickMessage').should('be.visible').and('have.text', 'You have done a dynamic click');
	...
	it('GX3-6095 | TC6: Validate that no results are returned with different actions on the “Right Click Me” Button', () => {
		cy.get('#rightClickBtn', { timeout: 90000 }).click();
