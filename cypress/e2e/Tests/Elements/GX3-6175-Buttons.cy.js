describe('GX3-6175 TOOLSQA ELEMENTS BUTTONS', () => {
	...
	it('US GX3-6175 | TC 1 | VALIDAR QUE BOTON  "DOUBLE CLICK" FUNCIONA CORRECTAMENTE ', () => {
		cy.get('#doubleClickBtn').should('be.visible').and('be.enabled').dblclick();
	});

	it('US GX3-6175|TC 2 | VALIDAR QUE BOTON "RIGHT CLICK" FUNCIONA CORRECTAMENTE ', () => {
		cy.get('button#rightClickBtn').should('be.visible').and('be.enabled').rightclick();
	});

	it('US GX3-6175 | TC 3 | VALIDAR QUE BOTON "CLICK ME" FUNCIONA CORRECTAMENTE', () => {
	...
	it('US GX3-6175 | TC 5 | VALIDAR QUE SE MUESTRE EL MENSAJE AL HACER RIGHT CLICK EN "BOTON RIGHT CLICK" ', () => {
		cy.get('button#rightClickBtn').should('be.visible').and('be.enabled').rightclick();
		cy.get('p[id=rightClickMessage]').should('be.visible').and('contain', 'You have done a right click');
	});

	it('US GX3-6175 | TC 6 | VALIDAR QUE SE MUESTRE MENSAJE  AL HACER CLICK EN BOTON "CLICK ME" ', () => {
		cy.get('.btn.btn-primary:not(#doubleClickBtn):not(#rightClickBtn)').should('be.visible').and('be.enabled').click();
