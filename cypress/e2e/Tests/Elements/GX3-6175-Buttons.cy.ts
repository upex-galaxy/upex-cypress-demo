describe('GX3-6105 TOOLSQA ELEMENTS BUTTONS', () => {
	...
	it('US GX3-6105 | TC 1 | VALIDAR HACER CLICK EN EL BOTÓN  DOUBLE CLICK', () => {
		cy.get('#doubleClickBtn').should('be.visible').and('be.enabled').dblclick();
	});

	it('US GX3-6105|TC 2 | VALIDAR HACER CLICK EN EL BOTÓN  RIGHT CLICK', () => {
		cy.get('button#rightClickBtn').should('be.visible').and('be.enabled').rightclick();
	});

	it('US GX3-6105 | TC 3 | VALIDAR HACER CLICK EN EL BOTÓN  CLICK ME', () => {
		cy.get('.btn.btn-primary:not(#doubleClickBtn):not(#rightClickBtn)').should('be.visible').and('be.enabled').click();
	});

	it('US GX3-6105 | TC 4 | VALIDAR QUE SE MUESTRE EL MENSAJE AL HACER CLICK EN EL BOTON  DOUBLE CLICK  ', () => {
		cy.get('#doubleClickBtn').should('be.visible').and('be.enabled').dblclick();
		cy.get('#doubleClickMessage').should('be.visible').and('contain', 'You have done a double click');
	});
	it('US GX3-6105 | TC 5 | VALIDAR QUE SE MUESTRE EL MENSAJE AL HACER CLICK EN BOTON RIGHT CLICK ', () => {
		cy.get('button#rightClickBtn').should('be.visible').and('be.enabled').rightclick();
