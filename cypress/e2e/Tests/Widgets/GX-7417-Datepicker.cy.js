...
describe('✅ToolsQA | Widgets | Date Picker', ()=>{
	...
	it('TC1: Validar seleccionar correctamente fecha', ()=>{
        ...
        datepicker.get.monthSelect().children().then(($month) =>{
            cy.randomNumber($month.length).then(($monthNumber) => {
                datepicker.selectMonth($monthNumber)
					cy.wrap($month).eq($monthNumber).then(($monthText) => {
                        cy.wrap($monthText).invoke('text').then(($resp)=>{
        ...
        datepicker.get.yearSelect().children().then(($year)=>{	
            ...
            cy.randomNumber($day.length).then(($dayNumber) => {
                cy.wrap($day).eq($dayNumber).click().then(($dayText)=>{
                    ...
                    datepicker.get.dateInput().then(($input)=>{
                        cy.wrap($input).invoke('val').then(($inputVal)=>{
    ...
    datepicker.clickOnDateAndTime()
        ...
        datepicker.get.timeSelect().children().then(($time) => {
            cy.randomNumber($time.length).then(($timeNumber) => {
                cy.wrap($time).eq($timeNumber).then(($timeText) => {
                cy.wrap($timeText).click()
				cy.wrap($timeText).invoke('text').then(($resp)=>{
                    ...
                    datepicker.get.dateAndTimeInput().should('have.value', `${mesLetra} ${dayF}, ${yearF} ${number}:${cade2} ${modo}`)
                    })
                })
            })
         })
