...
describe('GX277 | ToolsQA | Elements | Broken Links Images', () => {
    ...
    it('US 277 | TS 287 | TC1: Validation of an invalid image.', () => {
        cy.fixture("DOM/Elements/BrokenLinks.Page.json").then((the) => {
    ...
    it('US 277 | TS 287 | TC1: Validation of an invalid link.', () => { 
        cy.fixture("DOM/Elements/BrokenLinks.Page.json").then((the) =>
        {
            cy.get(the.link.invalid)
                .click()
            cy.url()
            .should('include', the.invalidUrl)
        })
    })
})