...
Cypress.Commands.add('dragAndDropX', (searchElement, x, y) => {
	cy.get(searchElement).move({deltaX: x, deltaY: y}).should('have.attr', 'style', `position: relative; left: ${x}px; top: 0px;`)
})
Cypress.Commands.add('dragAndDropY', (searchElement, x, y) => {
	cy.get(searchElement).move({deltaX: x, deltaY: y}).should('have.attr', 'style', `position: relative; left: 0px; top: ${y}px;`)
})
