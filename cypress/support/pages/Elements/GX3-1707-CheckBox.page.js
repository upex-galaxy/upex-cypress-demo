export default class CheckBoxPage {
	...
	async checkBoxAssigner(tree) {
		...
		return new Promise(resolve => {
			cy.wrap(null).then(() => {
				tree['selected'] = Math.random() < 0.2 ? 1 : 0;
				...
				for (const key1 in tree) {
					...
					if (tree['selected'] == 1) {
						...
						tree[key1]['selected'] = Math.random() < 0.3 ? 1 : 0;
					}
						...
						for (const key2 in tree[key1]) {
							...
							if (Array.isArray(tree[key1][key2])) {
								if (tree[key1]['selected'] == 1) {
									...
									tree[key1][key2]['selected'] = Math.random() < 0.4 ? 1 : 0;
								}
								...
								for (const key3 in tree[key1][key2]) {
									...
									if (tree[key1][key2]['selected'] == 1) {
										...
										tree[key1][key2][key3] = Math.random() < 0.5 ? 1 : 0;
									}
								}
							} else {
								if (tree[key1]['selected'] == 1) {
									...
									tree[key1][key2] = Math.random() < 0.5 ? 1 : 0;
								}
							}
						}
					}
