class FormPractice {
	...
	typeFirthName(firstName) {
		this.get.inputFirstName().type(firstName).then(firstName1 => {
	...
	typeMobile(mobileIn) {
		formPage.get.inputMobile().type(mobileIn).then(mobile => {
	...
	selectRandomsGender() {
		...
		this.get.optionsGenders().eq(randomsGe).click().then(genero => {
	...
	selectRandomHobbies() {
		...
		this.get.optionsHobbies().eq(randomsHo).click().then(hobbies1 => {
	...
	selectRandomState() {
		this.selectState();
		return this.get.selectStateAndCityOptions().its('length').then(cantElem => {
	...
	selectRandomCity() {
		this.selectCity();
		this.get.selectStateAndCityOptions().its('length').then(cantElem => {
			const randomsSt =Cypress._.random(0,cantElem-1);
			this.get.selectStateAndCityOptions().eq(randomsSt).click();
		});
	}

	typeRandomsSubjects(letraRandom) {
		this.get.inputSubjects().type(letraRandom);
		this.get.optionsSubjects().its('length').then(cant => {
			const randomsSt1 =Cypress._.random(0,cant-1);
			this.get.optionsSubjects().eq(randomsSt1).click();
			this.get.optionSubjectsText().invoke('text').then(subjects1 => {
	...
	selectRandomYearBirth() {
		this.get.selectYear().find('option').its('length').then(cant => {
	...
	selectRandomDayBirth() {
		this.get.selectDay().its('length').then(cant => {
