export class SpaceDestinationPage {
    ...
    getAnyCard() {
        return this.destinationCards().its('length').then((cardCount) => {
    ...
    getCardData(givenDesCard: JQuery<HTMLElement>) {
        ...
        cy.wrap(givenDesCard).within( () => {
            this.cardTitle().invoke('text').then(val => cardData.title = val)
            this.cardDesc().invoke('text').then(val => cardData.description = val)
            this.cardPrice().invoke('text').then(val => cardData.price = val)
